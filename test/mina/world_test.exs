defmodule Mina.WorldTest do
  ...
  describe "flag/2" do
    test "flags a mine", %{world: world} do
      assert World.flag(world, {0, 1}) == {:ok, %{{0, 1} => :flag}}
    end

    test "returns empty reveals on a proximity tile", %{world: world} do
      assert World.flag(world, {0, 0}) == {:error, :incorrect_flag}
    end

    test "returns empty reveals on an empty tile", %{world: world} do
      assert World.flag(world, {1, -2}) == {:error, :incorrect_flag}
    end

    test "does not reveal previous reveals", %{world: world} do
      reveals = %{{0, 1} => :flag}
      assert World.flag(world, {0, 1}, reveals: reveals) == {:error, :already_revealed}
    end
  end
end
